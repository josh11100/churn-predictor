...
.card-accent {
    background: linear-gradient(135deg, #0d1117 0%, #13151f 100%);
    border: 1px solid #1e2130;
    border-radius: 14px;
    padding: 1.8rem 2rem;
}
...
with right:
    if predict_btn:
        ...
        if pct >= 65:
            risk_label, risk_class, bar_color = "High Risk", "high", "#f87171"
        elif pct >= 35:
            risk_label, risk_class, bar_color = "Medium Risk", "medium", "#fbbf24"
        else:
            risk_label, risk_class, bar_color = "Low Risk", "low", "#4ade80"
        ...
        with m1:
            st.markdown(f"""<div class="card" style="text-align:center;">
                <div class="metric-val">${monthly:.0f}</div>
                <div class="metric-sub">Monthly at Risk</div>
            </div>""", unsafe_allow_html=True)
        with m2:
            st.markdown(f"""<div class="card" style="text-align:center;">
                <div class="metric-val">${monthly*12:.0f}</div>
                <div class="metric-sub">Annual at Risk</div>
            </div>""", unsafe_allow_html=True)
        with m3:
            st.markdown(f"""<div class="card" style="text-align:center;">
                <div class="metric-val">{tenure}mo</div>
                <div class="metric-sub">Tenure</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="section-label">Recommended Action</div>', unsafe_allow_html=True)
        actions = {
            "high":   "🔴 <strong>Intervene immediately.</strong> Offer a discount, loyalty reward, or personal outreach. This customer shows strong churn signals.",
            "medium": "🟡 <strong>Monitor closely.</strong> Send a proactive check-in or highlight unused features. A small gesture now prevents a bigger loss later.",
            "low":    "🟢 <strong>No action needed.</strong> This customer appears satisfied. Consider an upsell offer on an add-on service."
        }
        ...
        <div class="card-accent" style="text-align: center; padding: 3.5rem 2rem; margin-top: 1rem;">
            ...
            <div style="font-family: Syne, sans-serif; font-size: 1.3rem; font-weight: 700;
                        color: #f1f5f9; margin-bottom: 0.5rem;">Ready to analyze</div>
            <div style="color: #475569; font-size: 0.88rem; line-height: 1.7;
                        max-width: 300px; margin: 0 auto;">
                Fill in a customer profile on the left and click
                <strong style="color: #94a3b8;">Analyze Churn Risk →</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with p2:
            st.markdown("""<div class="card" style="text-align:center;">
                ...
                <div class="metric-sub">Churn Recall</div>
            </div>""", unsafe_allow_html=True)
        with p3:
            st.markdown("""<div class="card" style="text-align:center;">
                <div class="metric-val">7K</div>
                <div class="metric-sub">Training Rows</div>
            </div>""", unsafe_allow_html=True)
